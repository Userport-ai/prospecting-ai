...
from .config import TIMEOUTS, MAX_QUALIFICATION_SIGNALS
...
class ProspectingWorkflow:
    ...
    async def identify_qualification_signals(
        self, product: SellingProduct, research: str, interactive: bool = False
                ...
                for signal_data in signals_data[
                    ...
                    signals.append(
                        QualificationSignal(
                            ...
                            detection_instructions=signal_data.get(
    ...
    async def confirm_edit_signals(
        self, signals: List[QualificationSignal]
    ) -> List[QualificationSignal]:
        """
        ...
        for signal in signals:
            ...
            if response == "y":
                ...
                new_desc = (
                    input(f"Description [{signal.description[:50]}...]: ").strip()
                    or signal.description
                )
                new_imp = input(f"Importance [{signal.importance}]: ").strip() or str(
                    signal.importance
                )
                new_inst = (
                    input(
                        f"Detection [{signal.detection_instructions[:50]}...]: "
                    ).strip()
                    or signal.detection_instructions
                )

                signal.name = new_name
                signal.description = new_desc
                signal.importance = (
                    int(new_imp) if new_imp.isdigit() else signal.importance
                )
    ...
    def _create_qualification_steps(
        self, signals: List[QualificationSignal]
    ) -> List[ResearchStep]:
        ...
        for i, signal in enumerate(sorted_signals, 1):
            # Skip very low importance signals (1-2) if we already have enough high-importance ones
            if signal.importance <= 2 and i > MAX_QUALIFICATION_SIGNALS:
                if self.verbose:
                    print(
                        f"Skipping low-importance signal: {signal.name} (importance: {signal.importance})"
                    )
                continue
            ...
            Use multiple search queries and verify from multiple sources.
            """
            elif signal.importance == 3:
                signal_prompt += """
            This is an IMPORTANT signal - balance thoroughness with efficiency.
            Verify carefully with 1-2 targeted searches. Focus on clear evidence.
            """
            else:
                signal_prompt += """
            This is a SUPPLEMENTARY signal - verify quickly and efficiently.
            Use 1 targeted search. Report if easily found, otherwise note as "not readily available".
            """
    ...
    @staticmethod
    def _create_final_assessment_step(
    ...
    async def research_target(self, target: ProspectingTarget) -> ProspectingResult:
        ...
        # Always start with fresh base steps to avoid duplication
        self._setup_streamlined_steps()
        base_steps = self.research_steps.copy()
        ...
        if self.selling_product.qualification_signals:
            signal_steps = self._create_qualification_steps(
                self.selling_product.qualification_signals
            )
            all_steps.extend(signal_steps)

        # Add final assessment step
        final_step = self._create_final_assessment_step(
            self.selling_product.qualification_signals
        )
        all_steps.append(final_step)

        # Use local steps for this execution (don't modify self.research_steps)
        execution_steps = all_steps
        
        # Debug: Print all steps to check for duplicates
        if self.verbose:
            print(f"\n=== Total steps to execute: {len(execution_steps)} ===")
            for idx, step in enumerate(execution_steps):
                print(f"{idx+1}. {step.step_id}")
            print("=" * 50)

        # Initialize result
        result = ProspectingResult(target=target, selling_product=self.selling_product)

        # Execute steps with timeout
        try:
            async with asyncio.timeout(TIMEOUTS["account_research"]):
                for step in self._get_steps_in_execution_order(execution_steps):
                    ...
                    if self.verbose:
                        print(
                            f"Step {step.step_id} completed in {step_duration:.1f} seconds"
                        )
                        if step.step_id == "final_assessment":
                            print(">>> FINAL ASSESSMENT EXECUTED <<<")
                            print(f">>> Result contains {len(result.steps)} steps so far")
                    ...
                    if step_result.status in [
                        ...
                        print(
                            f"Step {step.step_id} {step_result.status}: {step_result.error}"
                        )
            ...
            print(
                f"Research for {target.company_name} timed out after {TIMEOUTS['account_research']} seconds"
            )
        ...
        try:
            ...
            for i, signal in enumerate(sorted_signals, 1):
                ...
                if (
                    ...
                    and result.steps[step_id].status == ResearchStatus.COMPLETED
                ):
                    ...
                    try:
                        ...
                        extracted = (
                            json_match.group(1) if json_match else response.content
                        )
                        ...
                        if (
                            "MATCH" in signal_text.upper()
                            and "NO MATCH" not in signal_text.upper()
                        ):
                            match_status = (
                                "partial" if "PARTIAL" in signal_text.upper() else "yes"
                            )
                        else:
                            match_status = "no"
            ...
            for match in signal_matches:
                ...
                for sig in self.selling_product.qualification_signals:
                    if (
                        sig.name.lower() in match.get("name", "").lower()
                        or match.get("name", "").lower() in sig.name.lower()
                    ):
                        original_signal = sig
                        break
                ...
                matched_signal = MatchedSignal(
                    name=match.get("name", ""),
                    importance=original_signal.importance
                    if original_signal
                    else match.get("importance", 3),
    ...
    def _calculate_research_quality(self, result: ProspectingResult):
        ...
        if self.selling_product.qualification_signals:
            ...
            total_importance = sum(
                s.importance for s in self.selling_product.qualification_signals
            )
            importance_score = (
                importance_weight / total_importance if total_importance > 0 else 0
            )
        ...
        if quality_score >= 0.8:
            ...
            result.validation_summary = (
                f"Moderate research quality: {quality_score:.0%} complete"
            )
        else:
            result.validation_summary = (
                f"Limited research: {quality_score:.0%} complete"
            )

    def _get_steps_in_execution_order(
        self, steps: List[ResearchStep] = None
    ) -> List[ResearchStep]:
        """Get research steps in proper execution order based on dependencies."""
        # Simple topological sort
        ordered = []
        remaining = (steps if steps is not None else self.research_steps).copy()
        ...
        while remaining:
            ...
            for step in remaining:
                if not step.depends_on:
                    ...
                    deps_met = all(
                        dep in [s.step_id for s in ordered] for dep in step.depends_on
                    )
                    if deps_met:
                        ready.append(step)
    ...
    def save_results(
        self, result: ProspectingResult, path: str = None, db_manager=None
    ):
    ...
    def _generate_simple_markdown(self, result: ProspectingResult) -> str:
        ...
        try:
            ...
            markdown += (
                f"- Website: [{result.target.website}]({result.target.website})\n"
            )
            ...
            if result.matched_signals:
                ...
                for signal in sorted(
                    result.matched_signals, key=lambda x: x.importance, reverse=True
                ):
                    markdown += (
                        f"- **{signal.name}** (Importance: {signal.importance}/5)\n"
                    )
