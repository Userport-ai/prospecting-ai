...
export const FileUpload: React.FC<{onFileUpload: (arg0: File) => void}> = ({ onFileUpload }) => {
  ...
  const handleDrop = (event: DragEvent<HTMLDivElement>) => {
    ...
    if(!event.dataTransfer) {
      console.error("No data transfer files found");
      return;
    }
  ...
  const handleSubmit = () => {
    if(!selectedFile) {
      console.error("Cannot submit since File not selected");
      return;
    }
...
const ImportCSV: React.FC<ImportCSVProps> = ({ open, onOpenChange, onImported }) => {
...
const AddAccountManually: React.FC<AddAccountManuallyProps> = ({ open, onOpenChange, onSuccessfulAdd }) => {
...
export default function AddAccounts() {
  ...
  const handleAccountAddedManually = (website: string) => {
